...
export async function handleAddToWishList(
  ...
  setWishListtCount: (count: number) => void
) {
...
class ApiServices {
  ...
  async checkOutByVisa(
    ...
    return await fetch(
      this.#baseUrl +
        ...
        cartId +
        "?url=https://veloratech.vercel.app",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  async checkOutByCash(
    cartId: string,
    address: string,
    phone: string,
    city: string
  ): Promise<IcheckOutByCash> {
    const enCodetoken = await getDecodedToken();
